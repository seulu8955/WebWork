...
<% 
	...
	/*
		...
		어떤 파일명으로 저장 되었는지 (saveFileName)
		업로드 당시 원본 파일명이 무었인지(orgFileName)
		파일의 사이즈는?
		다운로드 시켜주기위해서는 위의 3가지 정보가 필요하다.
	*/
	FileDto dto=FileDao.getInstance().getData(num);
	//3. 서버의 파일시스템(upload) 에 저장된 파일에서 바이트 알갱이를 읽어서 출력한다(다운로드)
	String orgFileName=dto.getOrgFileName();
    String saveFileName=dto.getSaveFileName();
	//다운로드 시켜줄 파일의 실제 경로 구성하기 윈도우 환경일지, 리눅스 혹은 맥 os 인지에 따라 분산자가 / \ 가 다르게 들어가야 하기때문에 하드코딩하지 않고 file 의 sepqrator 메소드를 사용함
	...
	//응답 헤더 정보 설정 응답 헤더 정보를 보고 저장할 파일 명을 정함 원본 파일명을 인코딩해서 넣어준것
	...
	//exception 발생하지 않도록(response.getOutputStream() 호출전에 해야함)
	out.clear();
